...
/**************************************/
// jobsInParallel() into Promise
const doJobsInParallelWithPromise = (jobId) => {
...
initJob = doJobsInParallelWithPromise(1);
initJob
  .then((result) => {
    return doJobsInParallelWithPromise(result + 1);
  })
  .then((result) => {
    return doJobsInParallelWithPromise(result + 1);
  })
...
//  doJobsAsynchronous() into Promise
const doJobsAsynchronousWithPromise = () => {
  return new Promise((resolve, reject) => {
    error = false;
    // Rejected
    if (error) {
...
initJob = doJobsAsynchronousWithPromise();
initJob
  .then(() => {
    return doJobsAsynchronousWithPromise();
  })
  .catch((error) => {
    console.log(error);
  });
...
Promise.all([orderFood, prepareFood, sendNotification]).then((values) => {
  console.log(values);
});
